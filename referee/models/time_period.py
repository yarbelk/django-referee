...
from ..managers import (CurrentTimePeriodManager,
                        CurrentAndPastTimePeriodManager)
...
class TimePeriodBase(models.Model):
    ...
    def clean(self, *args, **kwargs):
        ...
        if self.period_end <= self.period_start:
            raise ValidationError('period_end needs to be after period_start')
        ...
        for period in ('period_start', 'period_end'):
            ...
            if q.exists():
                raise ValidationError(
                    '{0} already in another period.'.format(period)
                )
        ...
        if(q.exists()):
            raise ValidationError('This period encompass another period.')
